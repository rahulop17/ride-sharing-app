...
int main() {
    int V, E;
    cin >> V >> E; // Number of nodes and edges in the road network
    ...
    int P;
    cin >> P; // Number of passengers
    vector<Passenger> passengers(P);
    set<int> relevantNodes;

    for (int i = 0; i < P; ++i) {
        int x1, x2;
        cin >> x1 >> x2; // pickup and dropoff
        passengers[i] = {x1, x2};
        relevantNodes.insert(x1);
        relevantNodes.insert(x2);
    }

    // Precompute all Dijkstra results
    unordered_map<int, unordered_map<int, int>> allDistances;
    for (int node : relevantNodes) {
        allDistances[node] = dijkstra(node, adj);
    }

    // Compute weights for all passenger pairs
    for (int i = 0; i < P; ++i) {
        for (int j = i + 1; j < P; ++j) {
            auto [X1, X2] = passengers[i];
            auto [Y1, Y2] = passengers[j];
            ...
            if (allDistances[X1].count(Y1) && allDistances[Y1].count(X2) && allDistances[X2].count(Y2)) {
                ...
                // Check level of service for both passengers
            ...
            // Scenario 5: X travels alone, Y travels alone
            int d5 = dXsolo + dYsolo;

            int finalDistance = min({d1, d2, d3, d4, d5});
